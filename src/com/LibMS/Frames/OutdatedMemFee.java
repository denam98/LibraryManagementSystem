 ...
 */
public class OutdatedMemFee extends javax.swing.JFrame {

    /**
     * Creates new form OutdatedMemFee
     */
    public OutdatedMemFee() {
        try {
            UIManager.setLookAndFeel(UIManager.getSystemLookAndFeelClassName());
        } catch (ClassNotFoundException | IllegalAccessException | InstantiationException | UnsupportedLookAndFeelException ex) {
            SystemLogger.initLogger(OutdatedMemFee.class).error("Exception from class constructor", ex);
        }
        initComponents();
        setLocationRelativeTo(null);
        setImage(new File("pics/outdatedPay.png"), lab_image);
        loadTableData();
        setButtons();
        jLabel3.grabFocus();
    }
    ...
    private void initComponents() {
        ...
        jTable1.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "MEMBER ID", "FIRST NAME", "LAST NAME", "GENDER", "MOBILE", "LAST PAID", "PENDING FOR"
            }
        ) {
        ...
        cb_searchCategory.setModel(new javax.swing.DefaultComboBoxModel<>(new String[] { "ID", "First Name", "Last Name", "Mobile No." }));
        ...
        but_addMem.setText("Fee Payment History");
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            SystemLogger.initLogger(OutdatedMemFee.class).error("Exception from main() method", ex);
        }
        //</editor-fold>
    ...
    private void loadTableData() {
        ...
        try {
            ResultSet search1 = DBConnect.search("select mf.id as payid,mf.*,m.* from member_fee mf left join member m "
                    + "on mf.member_id=m.id where mf.year<EXTRACT(YEAR FROM NOW()) order by mf.member_id");
            int memid = 0;
            int year = 0;
            int paymentid = 0;
            int x = 1;
            while (search1.next()) {
                //System.out.println(search1.getString("member_id") + " - " + search1.getString("fname") + " - " + search1.getString("lname") + " - " + search1.getString("gender") + " - " + search1.getString("year"));

                if (search1.isFirst()) {
                    memid = search1.getInt("member_id");
                    year = search1.getInt("year");
                    paymentid = search1.getInt("payid");
                } else {

                    if (memid == search1.getInt("member_id")) {
                        if (year < search1.getInt("year")) {
                            year = search1.getInt("year");
                            paymentid = search1.getInt("payid");
                        }
                    } else {
                        ResultSet search = DBConnect.search("select mf.id as payid,mf.*,m.* from member m inner join member_fee mf "
                                + "on mf.member_id=m.id where mf.id='" + paymentid + "'");
                        v = new Vector();
                        if (search.first()) {
                        ...
                        year = search1.getInt("year");
                        paymentid = search1.getInt("payid");
                        //System.out.println("Loop no." + x + ". Data raw added.\n Current mem id - " + memid + "\n Current payment id - " + paymentid + "\n Current year - " + year);
                    }
            ...
            ResultSet search2 = DBConnect.search("select * from member m where NOT EXISTS (select * from member_fee mf where m.id=mf.member_id)");
            while (search2.next()) {
                v = new Vector();
                v.add(search2.getString("id"));
                v.add(search2.getString("fname"));
                v.add(search2.getString("lname"));
                String gender = (search2.getString("gender").equals("M")) ? "Male" : "Female";
                v.add(gender);
                v.add(search2.getString("phone"));
                v.add("Null");
            ...
            SystemLogger.initLogger(OutdatedMemFee.class).error("", ex);
        }
    }
